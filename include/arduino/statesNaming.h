...
static String getResponseDescriptionByParams(byte status){
	switch (status)
	{
	  ...
	  case ERR_E220_NOT_SUPPORT:
		return F_("Not support!");
		break;
	  case ERR_E220_NOT_IMPLEMENT:
		return F_("Not implement");
		break;
	  case ERR_E220_NOT_INITIAL:
		return F_("Not initial!");
		break;
	  case ERR_E220_INVALID_PARAM:
		return F_("Invalid param!");
		break;
	  case ERR_E220_DATA_SIZE_NOT_MATCH:
		return F_("Data size not match!");
		break;
	  case ERR_E220_BUF_TOO_SMALL:
		return F_("Buff too small!");
		break;
	  case ERR_E220_TIMEOUT:
		return F_("Timeout!!");
		break;
	  case ERR_E220_HARDWARE:
		return F_("Hardware error!");
		break;
	  case ERR_E220_HEAD_NOT_RECOGNIZED:
		return F_("Save mode returned not recognized!");
		break;
	  case ERR_E220_NO_RESPONSE_FROM_DEVICE:
		return F_("No response from device! (Check wiring)");
		break;
	  case ERR_E220_WRONG_UART_CONFIG:
		return F_("Wrong UART configuration! (BPS must be 9600 for configuration)");
		break;
	  case ERR_E220_PACKET_TOO_BIG:
		return F_("The device support only 200byte of data transmission!");
		break;
	  default:
		return F_("Invalid status!");
	}
}
...
static String getUARTParityDescriptionByParams(byte uartParity){
	switch (uartParity)
	{
	  ...
	  case MODE_10_8E1:
		...
		break;
	  case MODE_11_8N1:
		return F_("8N1 (equal to 00");
		break;
	  default:
		return F_("Invalid UART Parity!");
	}
}
...
static String getUARTBaudRateDescriptionByParams(byte uartBaudRate)
{
	switch (uartBaudRate)
	{
	  ...
	  case UART_BPS_115200:
		...
		break;
	  default:
		return F_("Invalid UART Baud Rate!");
	}
}
...
static String getAirDataRateDescriptionByParams(byte airDataRate)
{
	switch (airDataRate)
	{
	  ...
	  case AIR_DATA_RATE_111_625:
		...
		break;
	  default:
		return F_("Invalid Air Data Rate!");
	}
}
...
static String getSubPacketSettingByParams(byte subPacketSetting)
{
	switch (subPacketSetting)
	{
	  ...
	  case SPS_032_11:
		...
		break;
	  default:
		return F_("Invalid Sub Packet Setting!");
	}
}
...
static String getRSSIAmbientNoiseEnableByParams(byte rssiAmbientNoiseEnabled)
{
	switch (rssiAmbientNoiseEnabled)
	{
	  ...
	  case RSSI_AMBIENT_NOISE_DISABLED:
		  return F_("Disabled (default)");
		break;
	  default:
		return F_("Invalid RSSI Ambient Noise enabled!");
	}
}
...
static String getLBTEnableByteByParams(byte LBTEnableByte)
{
	switch (LBTEnableByte)
	{
	  ...
	  case LBT_DISABLED:
		...
		break;
	  default:
		return F_("Invalid LBT enable byte!");
	}
}
...
static String getRSSIEnableByteByParams(byte RSSIEnableByte)
{
	switch (RSSIEnableByte)
	{
	  ...
	  case RSSI_DISABLED:
		...
		break;
	  default:
		return F_("Invalid RSSI enable byte!");
	}
}
...
static String getFixedTransmissionDescriptionByParams(byte fixedTransmission)
{
	switch (fixedTransmission)
	{
	  case FT_TRANSPARENT_TRANSMISSION:
		return F_("Transparent transmission (default)");
		break;
	  case FT_FIXED_TRANSMISSION:
		return F_("Fixed transmission (first three bytes can be used as high/low address and channel)");
		break;
	  default:
		return F_("Invalid fixed transmission param!");
	}
}
	...
	static String getTransmissionPowerDescriptionByParams(byte transmissionPower)
	{
		switch (transmissionPower)
		{
		  ...
		  case POWER_10:
			...
			break;
		  default:
			return F_("Invalid transmission power param");
		}
	}
#endif
